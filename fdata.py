...
if view =='Fighter One Sheet':
    ...
    td_rate = duckdb.sql("SELECT sum(td_l::INTEGER)/cast(sum(td_a::REAL) as REAL) s from fighter_stats").df()
    ...
    if fighter_filter:
        ...
        with col3:
            ...
            st.write(str(int(td['s'].sum()))+' Total Takedowns Landed'+' at a rate of '+str(float(td_rate['s'].sum())))
