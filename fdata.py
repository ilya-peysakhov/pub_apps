...
if view=='Welcome':
  ...
  st.image('https://media.tenor.com/8jkYjD4cnqUAAAAM/just-bleed.gif')
    ...
    if fighter_filter:
        ...
        st.divider()
        cumulative_head_trauma = duckdb.sql(f"SELECT date, sum(head_str_l::int) head_str_l from fs_cleaned where BOUT in (select * from fights) and FIGHTER !='{fighter_filter}' group by 1 ").df()
