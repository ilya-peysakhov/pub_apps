...
if view =='Fighter One Sheet':
    ...
    st.write(duckdb.sql("select DATE,EVENT, count(BOUT) as bouts_with_stats from ed_c left join fs on ed_c.EVENT =fs.EVENT group by 1,2 order by 1 desc limit 10").df())
else:
    c1, c2 = st.columns(2)
    with c1:
