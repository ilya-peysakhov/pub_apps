...
if view =='Fighter One Sheet':
    ...
    if fighter_filter:
        ...
        with col4:
            ...
            st.metric('Total Takedowns Landed',value=int(cleaned_fighter_stats['td_l']),delta="{0:.0%}".format(round(float(cleaned_fighter_stats['td_rate'].iloc[0]),2)), delta_color="normal")
            st.metric('Total Takedowns Given Up',value=int(cleaned_opp_stats['td_abs']),delta="{0:.0%}".format(round(float(cleaned_opp_stats['td_abs_rate'].iloc[0]),2)), delta_color="inverse")
        with col5:
