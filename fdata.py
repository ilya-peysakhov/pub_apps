...
if view =='Fighter One Sheet':
    ...
    anomalies = duckdb.sql("select DATE,ed_c.EVENT, count(BOUT) as bouts_with_stats from ed_c left join fs on ed_c.EVENT =fs.EVENT group by 1,2 order by 1 desc having bouts_with_stats=0").df()
