...
if view =='Fighter One Sheet':
    ...
    for f in fighters['FIGHTER']:
        query = f"SELECT '{f}' AS FIGHTER,count(distinct BOUT||EVENT) as FIGHTS,count(*) as ROUNDS, SUM(head_str_l::INTEGER) AS HEAD_STRIKES_ABSORED,sum(KD::INTEGER) as KD, sum(TD_L::INT) as TD, ROUNDS/FIGHTS as ROUNDS_PER_FIGHT FROM fs_cleaned WHERE BOUT LIKE '%{f}%' AND fighter != '{f}' "
