...
if view=='Welcome':
  ...
  st.image('https://media.tenor.com/8jkYjD4cnqUAAAAM/just-bleed.gif')
    ...
    with c2:
        ...
        fight_distro = duckdb.sql("""select FIGHTS, SUM(FIGHTERS) OVER (ORDER BY FIGHTS asc) cumulative_sum from
                                  (select FIGHTS,count(1) FIGHTERS from (select FIGHTER,COUNT(DISTINCT EVENT||BOUT) FIGHTS from fs_cleaned group by 1) )
