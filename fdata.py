...
if view =='Fighter One Sheet':
    ...
    with c2:
        ...
        st.table(duckdb.sql("SELECT LOCATION,count(distinct EVENT) events from fed where date between current_date() -730 and current_date() group by 1 order by 2 desc limit 10").df())
