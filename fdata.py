...
if view =='Fighter One Sheet':
    ...
    if fighter_filter:
        ...
        with c1:
            ...
            fig = px.area(str_a, x='DATE', y='Total_Strikes_At', template='plotly_white')
            ...
            fig = px.area(str_dif, x='DATE', y='Strike_Diff', template='plotly_white')
            st.plotly_chart(fig,use_container_width=True)
              
        with c2:
            st.write("Takedowns Attempted")
            td_a = duckdb.sql(f"SELECT DATE,  sum(td_a::int) TD_At from fighter_stats group by 1").df()
            fig = px.area(td_a, x='DATE', y='TD_At', template='plotly_white')
            st.plotly_chart(fig,use_container_width=True)
            ...
            fig = px.area(td_dif, x='DATE', y='TD_At_Diff', template='plotly_white')
            st.plotly_chart(fig,use_container_width=True)
