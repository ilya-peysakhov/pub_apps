...
if view =='Fighter One Sheet':
    ...
    if fighter_filter:
        ...
        with col5:
            ...
            st.metric('Head Movement',value=round({0:.0%}".format(1-(cleaned_opp_stats['head_abs']/cleaned_opp_stats['head_at'],2))))
        c1, c2 = st.columns(2)
        with c1:
    ...
    combined_stats = duckdb.sql("SELECT a.*, ROUND(SIG_STRIKES_LANDED/SIG_STRIKES_ABS,1) as SIG_STR_DIFF, ROUND(HEAD_STRIKES_ABS/HEAD_STRIKES_AT,2) as HEAD_MOVEMENT, b.* EXCLUDE (FIGHTER) from all_time_offense as a left join str_results as b on a.FIGHTER=b.FIGHTER").df()
