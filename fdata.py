...
if view =='Fighter One Sheet':
    ...
    if fighter_filter:
        ...
        with col1:
            ...
            st.write(str(int(ko_wins['s']))+' KO/TKO Wins')
            st.write(str(int(ko_losses['s']))+' KO/TKO Losses')
        with col2:
            st.subheader('Striking')
            st.write(str(int(cleaned_opp_stats['sig_abs']))+' Significant Strikes Absored')
            st.write(str(int(cleaned_opp_stats['head_abs']))+' Head Strikes Absored')
            st.write(str(int(cleaned_fighter_stats['sig_str']))+' Significant Strikes Landed')
            st.write(str(int(cleaned_fighter_stats['head_str']))+' Head Strikes Landed')
            st.write(str(int(cleaned_fighter_stats['kd']))+' Knockdowns Landed')
    ...
    for f in fighters['FIGHTER']:
        query = f"SELECT '{f}' AS FIGHTER,count(distinct EVENT) as FIGHTS,count(*) as ROUNDS, SUM(head_str_l::INTEGER) AS HEAD_STRIKES_ABSORED,sum(KD::INTEGER) as KD, sum(TD_L::INT) as TD FROM fs_cleaned WHERE BOUT LIKE '%{f}%' AND fighter != '{f}' "
