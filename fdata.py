...
if view =='Fighter One Sheet':
    ...
    for f in fighters['FIGHTER']:
        query = f"SELECT '{f}' AS FIGHTER, SUM(head_str_l::INTEGER) AS HEAD_STRIKES_ABSORED, round(FLOAT(HEAD_STRIKES_ABSORED/SUM(head_str_a::INTEGER)),2) as HEAD_MOVEMENT ,SUM(sig_str_l::INTEGER) AS SIG_STRIKES_ABSORBED,SUM(leg_str_l::INTEGER) as LEG_STRIKES_ABSORBED,sum(KD::INTEGER) as KD_ABSORED, sum(TD_L::INT) as TD_GIVEN_UP FROM fs_cleaned WHERE BOUT LIKE '%{f}%' AND fighter != '{f}' "
